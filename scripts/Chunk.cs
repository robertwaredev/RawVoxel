...
namespace RawVoxel
{
    ...
    public partial class Chunk : MeshInstance3D
    {
        ...
        private void GenerateBiome(Vector3I chunkGridPosition)
        {
            ...
            float temperatureDistribution = _world.TemperatureDistribution.Sample((float)(chunkGridPosition.Z + 0.5f) / _world.WorldDiameter.Z);
            ...
            float humidityDistribution = _world.HumidityDistribution.Sample((float)(chunkGridPosition.X + 0.5f) / _world.WorldDiameter.X);
            ...
            foreach (Biome biome in _world.BiomeLibrary.Biomes)
            {
                if (chunkTemperature <= biome.TemperatureMax && chunkTemperature >= biome.TemperatureMin &&
                    chunkHumidity <= biome.HumidityMax && chunkHumidity >= biome.HumidityMin)
                {
                    _biome = biome;
                }
                else
        ...
        public void Generate(int chunkGridIndex)
        {
            ...
            SetPosition(chunkGridPosition - _world.WorldRadius);
            GenerateBiome(chunkGridPosition);
            RawTimer.Time(GenerateVoxelIDs);
        ...
        private int GenerateVoxelID(Vector3I voxelGridPosition)
        {
            ...
            // Sample biome for height.
            float heightNoise = _biome.HeightNoise.GetNoise2D(voxelGlobalPosition.X, voxelGlobalPosition.Z);
        ...
        public void SetVoxelID(Vector3I voxelGridPosition, int voxelID)
        {
            ...
            VoxelIDs[XYZConvert.Vector3IToIndex(voxelGridPosition, _world.ChunkDimension)] = (byte)voxelID;
        }
        ...
        private void GenerateChunkMeshSurface()
        {
            ...
            // Add surface to arrayMesh using surfaceArray to populate its data.
